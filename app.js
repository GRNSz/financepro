...
function renderDashboard() {
  ...
  // ⚡ Bolt: Fast string prefix match instead of Date parsing
...
function cardInvoice(cardId, year, month) {
  ...
  S.transactions.forEach(t => {
    ...
    // ⚡ Bolt: Fast substring date extraction instead of Date parsing
    let iy = parseInt(t.date.substring(0, 4), 10);
    let im = parseInt(t.date.substring(5, 7), 10) - 1;
    let id = parseInt(t.date.substring(8, 10), 10);
...
function applyFilters() {
  ...
  const thisMonthPrefix = `${cy}-${String(cm + 1).padStart(2, '0')}-`;
  ...
  const filtered = S.transactions.filter(t => {
    ...
    if (period !== 'all') {
      if (period === 'thisMonth' && !t.date.startsWith(thisMonthPrefix)) return false;
      if (period === 'lastMonth' && !t.date.startsWith(lastMonthPrefix)) return false;
    }
    return true;
...
function renderBudgets() {
  ...
  S.transactions.forEach(t => {
    if (t.type !== 'expense') return;
    if (t.date.startsWith(monthPrefix)) spent[t.categoryId] = (spent[t.categoryId] || 0) + t.amount;
  });
